...
inline void gcode_M141() {
  if (DEBUGGING(DRYRUN)) return;
    uint16_t s = code_seen('S') ? code_value_ushort() : 255;
    NOMORE(s, 255);
    if (s < 255) WRITE(HEATER_CHAMBER_PIN, LOW);
    if (s = 255) WRITE(HEATER_CHAMBER_PIN, HIGH);
}
...
void process_next_command() {
  ...
  switch (command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      case 141: // M141: Set chamber heater
        gcode_M141();
        break;
